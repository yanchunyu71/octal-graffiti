...
def load_images(image_paths, convert=None):
    iter_all_images = (cv2.resize(cv2.imread(fn), (256, 256)) for fn in image_paths)# 把读入的图片装换为256*256的尺寸
    if convert:
        iter_all_images = (convert(img) for img in iter_all_images)
    for i, image in enumerate(iter_all_images):
        if i == 0:
            all_images = numpy.empty((len(image_paths),) + image.shape, dtype=image.dtype)#根据给定的维度和数值类型返回一个新的数组（空数组的维度，数组的数值类型）
        all_images[i] = image
    return all_images#返回装满了数据的数组
