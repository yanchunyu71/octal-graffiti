...
def toTensor(img):
    img = torch.from_numpy(img.transpose((0, 3, 1, 2)))#transpose将image图片的序列由0,1,2,3改变为0,3,1,2
    return img                                         #from_numpy：Creates a Tensor from a numpy.ndarray.并且该tensor与numpy占用相同内存空间


def var_to_np(img_var):
    return img_var.data.cpu().numpy()#首先img_var是一个放在GPU上的Variable，img_var.data是把Variable里的tensor取出来
                                     #然后.cpu()是把img_var.data放在cpu上    最后.numpy()是把tensor转换为numpy
